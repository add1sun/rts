...
class SI_LocalSearches extends Pepper
{
	...
	function onRecord()
	{
		...
		$found	= 0;
 		...
 		if (preg_match('http://rocktreesky.com/?q=search/node/', html_entity_decode($referer), $matches))
		{
 		...
 		else if (preg_match("/?q=search/node/", html_entity_decode($resource), $matches))
		{
			$match	= $matches[count($matches) - 1];
 			$terms = $this->escapeSQL(rawurldecode(htmlentities($match)));
 		}
