...
function basicrts_preprocess_html(&$vars) {
  ...
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = basicrts_id_safe('page-'. $path);
    $vars['classes_array'][] = basicrts_id_safe('section-'. $section);
  ...
  // Add IE classes.
  if (theme_get_setting('basicrts_ie_enabled')) {
    $basicrts_ie_enabled_versions = theme_get_setting('basicrts_ie_enabled_versions');
    if (in_array('ie8', $basicrts_ie_enabled_versions, TRUE)) {
...
function basicrts_preprocess_page(&$vars, $hook) {
...
function basicrts_preprocess_node(&$vars) {
  ...
  // Trim submitted output.
  $vars['submitted'] = 'Posted on ' . format_date($vars['created'], 'custom', 'd F Y');

  // Merge first/last class (from basicrts_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
  if (!empty($node->classes_array)) {
    $vars['classes_array'] = array_merge($vars['classes_array'], $node->classes_array);
  }

  // Add a fricking comment link on a full node.
  $vars['comment_link'] = l('Add a new comment', 'comment/reply/' . $node->nid, array('fragment' => 'comment-form'));
}

function basicrts_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function basicrts_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('basicrts_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('basicrts_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('basicrts_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('basicrts_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('basicrts_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */
function basicrts_id_safe($string) {
 ...
 *
 */
function basicrts_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = basicrts_id_safe($element['#title']);
 ...
 */
function basicrts_preprocess_menu_local_task(&$variables) {
 ...
 */
function basicrts_menu_local_tasks(&$variables) {
  $output = '';
