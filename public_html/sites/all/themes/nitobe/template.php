...
require_once path_to_nitobetheme() . '/nitobe_utils.php';


/**
 * Return the path to the main nitobe theme directory.
 */
function path_to_nitobetheme() {
  static $theme_path;
...
function nitobe_preprocess_user_picture(&$variables) {
  ...
  if (variable_get('user_pictures', 0)) {
    ...
    if (!empty($account->picture) && file_exists($account->picture)) {
      ...
      $picture = path_to_nitobetheme() . '/user-icon.jpg';
    }
 ...
 */
function nitobe_preprocess_user_profile(&$vars) {
...
function nitobe_alt_word_text($text = '') {
  if ((boolean)theme_get_setting('nitobe_header_spaces')) {
    return $text;  
  }
...
function phptemplate_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
    if (!(boolean)theme_get_setting('nitobe_show_single_crumb')) {
        if (count($breadcrumb) == 1) {
            return '';
        }
    }

    $glue = theme_get_setting('nitobe_breadcrumb_separator');
 ...
 * @return string The rendered HTML.
 */
function nitobe_links($links, $attributes = array('class' => 'links'), $context = '') {
  if ($context == 'comment-meta') {
    return _nitobe_comment_meta_links($links);
  }
  else {
 ...
 *
 */
function _nitobe_comment_meta_links($links, $attributes = array('class' => 'links')) {
  $titles    = array(
    'comment_edit'   => t('Edit'),
  ...
  $suppress = (theme_get_setting('nitobe_suppress_comment_reply') == 1);
  ...
  $glue = theme_get_setting('nitobe_link_separator');
  $glue = ($glue !== NULL) ? $glue : ' | ';
 ...
 */
function nitobe_user_register($form) {
  $output = '';
  //$output .= print_r($form);
 ...
 */
function nitobe_gmap_location_node_page($count, $header, $map, $footer) {
  $output = '';
  if ($header) {
    $output .= "<p>$header</p>";
  }
